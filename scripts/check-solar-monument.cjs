...
const { sunDirection, blockShadow, contains } = context.window.SolarGeometry;
...
new vm.Script(fs.readFileSync(path.join(root, 'game.js'), 'utf8'));
console.log('Solar monument: geometry, cardinal bearings, overhead/low/night Sun, lintel gaps, scale, seasonal altitude, event dates, and script syntax passed.');
