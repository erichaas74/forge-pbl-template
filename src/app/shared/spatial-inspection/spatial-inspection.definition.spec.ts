...
describe('Spatial inspection configuration',()=>{
  it('validates optional scene-specific labels without requiring them for older definitions',()=>{
    expect(requireSpatialInspection({...definition,title:'Cultivated field',overviewLabel:'Whole field'}).title).toBe('Cultivated field');
    expect(()=>requireSpatialInspection({...definition,title:''})).toThrow('INVALID_SPATIAL_INSPECTION');
  });
