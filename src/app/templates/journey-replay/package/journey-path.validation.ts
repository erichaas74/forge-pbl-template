...
export function validateJourneyPaths(
  value: unknown,
  map: JourneyMapConfig,
  resources: readonly JourneyResourceDefinition[],
  evidence: readonly JourneyEvidenceDefinition[],
): ValidationIssue[] {
  if (value === undefined) return [];
  const issues: ValidationIssue[] = [];
  const error = (message: string) =>
    issues.push({ code: 'JOURNEY_PATH_INVALID', severity: 'error', file: 'journey.json', message });
  if (
    !record(value) ||
    value['schemaVersion'] !== '1.0' ||
    value['capability'] !== 'branchingJourney' ||
    !text(value['startNodeId']) ||
  ...
  for (const node of value['nodes']) {
    if (
      !record(node) ||
      !['id', 'title', 'locationId', 'product', 'learning'].every((key) => text(node[key])) ||
      !Number.isInteger(node['session']) ||
      Number(node['session']) < 1 ||
      Number(node['session']) > 8 ||
      !['map', 'location'].includes(String(node['kind'])) ||
      !strings(node['tasks']) ||
      !node['tasks'].length ||
    ...
    if (
      node['sceneArt'] !== undefined &&
      (!record(node['sceneArt']) ||
        !['backdrop', 'ship'].every((key) => {
          const art = node['sceneArt'] as Record<string, unknown>;
          return (
            typeof art[key] === 'string' &&
            /^\/[a-zA-Z0-9/_.-]+\.(webp|png|jpg)$/.test(String(art[key]))
          );
        }))
    )
    ...
    if (
      node['kind'] === 'map'
        ? node['choices'].length < 1 || node['events'].length !== 0
        : node['events'].length < 1 ||
          node['choices'].length !== 0 ||
          !['island', 'harbor', 'storm', 'river', 'cape', 'home'].includes(String(node['scene']))
    )
    ...
    for (const event of node['events']) {
      if (
        !record(event) ||
        !['id', 'label', 'observation', 'question'].every((key) => text(event[key])) ||
        !strings(event['evidenceIds']) ||
        !event['evidenceIds'].every((id) => validEvidence.has(id)) ||
        !validObject(event['object']) ||
    ...
    const choices = [
      ...node['choices'],
      ...node['events'].flatMap((event) =>
        record(event) && Array.isArray(event['choices']) ? event['choices'] : [],
      ),
    ];
    for (const choice of choices) {
      if (
        !record(choice) ||
        !['id', 'label', 'summary', 'consequence', 'nextTask', 'learning'].every((key) =>
          text(choice[key]),
        )
      ) {
      ...
      for (const key of ['grants', 'requiresAny'])
        if (choice[key] !== undefined && !strings(choice[key]))
          error(`Invalid tags in ${choice['id']}.`);
      if (
        choice['effect'] !== undefined &&
        !['water', 'repair', 'charts', 'rest', 'exchange', 'sail'].includes(
      ...
      if (
        ...
        (!record(changes) ||
          Object.entries(changes).some(
            ([id, amount]) =>
              !validResources.has(id) || typeof amount !== 'number' || !Number.isFinite(amount),
          ))
      )
        error(`Invalid resources in ${choice['id']}.`);
      if (
        choice['consequenceModifiers'] !== undefined &&
        (!Array.isArray(choice['consequenceModifiers']) ||
          choice['consequenceModifiers'].some(
            (modifier) =>
              !record(modifier) ||
              !text(modifier['afterChoiceId']) ||
              !text(modifier['narrative']) ||
              !record(modifier['resourceChanges']) ||
              Object.entries(modifier['resourceChanges']).some(
                ([id, amount]) =>
                  !validResources.has(id) || typeof amount !== 'number' || !Number.isFinite(amount),
              ),
          ))
  ...
  const granted = new Set(
    definition.nodes
      .flatMap((node) => [...node.choices, ...node.events.flatMap((event) => event.choices)])
    ...
    for (const choice of choices) {
      ...
      if (node.kind === 'map') {
        ...
        if (
          !choice.nextNodeId ||
          !route ||
          route.fromLocationId !== node.locationId ||
          route.toLocationId !== nodes.get(choice.nextNodeId)?.locationId
        )
