...
import type {
  JourneyWorldCallbacks,
  JourneyWorldHandle,
  JourneyWorldView,
} from './journey-world.contracts';
...
export function mountJourneyLocationWorld(
  parent: HTMLElement,
  ...
  class Anchorage extends Phaser.Scene {
    ...
    create(): void {
      ...
      this.shade = this.add.ellipse(0, 0, 600, 48, 0x031b24, 0.48);
      ...
      this.frontSail = this.add
        .image(-126, -754, damaged ? 'canvas-torn' : 'canvas-full')
        .setOrigin(0.5, 0)
        .setDisplaySize(490, 400);
      this.rearSail = this.add
        .image(328, -722, 'canvas-full')
        .setOrigin(0.5, 0)
        .setDisplaySize(402, 288);
      this.ship.add([
        this.frontSail,
        this.rearSail,
      ...
      for (let i = 0; i < 3; i++) {
        this.timber
          .fillStyle(0x9d7549)
          .fillRoundedRect(-140, -259 + i * 13, 220, 12, 2)
          .lineStyle(2, 0xd5af70, 0.6)
          .lineBetween(-130, -255 + i * 13, 72, -255 + i * 13);
      }
      this.ship.add(this.timber);
      for (let i = 0; i < 4; i++) {
        const barrel = this.add
          .image(-225 + i * 74, -276, 'journey-barrel')
          .setDisplaySize(59, 72)
          .setAlpha(i === 0 ? 1 : 0);
      ...
      paper
        .lineStyle(2, 0x4b7167, 0.9)
        .lineBetween(210, -289, 252, -315)
        .lineBetween(252, -315, 294, -292);
      this.ship.add(paper);
      this.tender = this.add.container(0, 0);
      const boat = this.add.graphics();
      boat.fillStyle(0x112733, 0.5).fillEllipse(0, 15, 137, 22);
      boat.fillStyle(0x553d27, 1).fillPoints(
        [
          { x: -74, y: -12 },
          { x: -42, y: 18 },
          { x: 47, y: 15 },
      ...
      boat
        .lineStyle(3, 0xc5a16c, 1)
        .lineBetween(-73, -13, -32, -4)
        .lineBetween(-32, -4, 33, -6)
        .lineBetween(33, -6, 77, -19);
      boat.lineStyle(5, 0x8c6d43, 1).lineBetween(-11, -10, 43, 44);
    ...
    private layout(): void {
      ...
      this.ship
        .setPosition(this.shipBase.x, this.shipBase.y)
        .setScale(Math.min(shipWidth / 1536, (h * 0.7) / 1024));
      this.shade
        .setPosition(this.shipBase.x, this.shipBase.y - 6)
        .setDisplaySize(shipWidth * 0.7, Math.max(15, shipWidth * 0.045));
      this.tender
        .setPosition(w * 0.76, h * 0.63)
        .setScale(Math.max(0.45, Math.min(0.85, w / 1100)));
      this.effectKey = '';
    }
    override update(_time: number, delta: number): void {
      ...
      this.ship
        .setAngle(moving ? Math.sin(t * 0.75) * strength : 0)
        .setY(this.shipBase.y + (moving ? Math.sin(t * 1.05) * 3.5 : 0));
    ...
    private drawWater(t: number, w: number, h: number, strength: number): void {
      ...
      for (let i = 0; i < 48; i++) {
        ...
        const y = h * (0.48 + depth * 0.5);
        const x = ((i * 163 + t * (8 + depth * 15)) % (w + 80)) - 40;
        const width = 8 + depth * 35;
        g.lineStyle(
          0.5 + depth,
          rough ? 0xc4e9e6 : 0xffedbc,
          (0.08 + Math.sin(t * 0.6 + i) * 0.04) * depth,
        );
        g.beginPath();
        g.moveTo(x, y);
        g.lineTo(x + width * 0.4, y - 1.5 * strength);
        g.lineTo(x + width, y);
        g.strokePath();
      }
      const x = this.shipBase.x,
        y = this.ship.y - 8,
        sw = this.ship.scaleX * 1536;
      for (let i = 0; i < 3; i++)
        g.lineStyle(1.4, 0xdbf7ec, 0.14 - i * 0.025).strokeEllipse(
          x + sw * 0.02,
          y + i * 5,
          sw * (0.68 + i * 0.05),
          10 + i * 9,
    ...
    private drawSurvey(w: number, h: number, visible: boolean): void {
      ...
      for (const end of [
        ...
        const distance = Math.hypot(end.x - start.x, end.y - start.y),
          steps = Math.ceil(distance / 15);
        g.lineStyle(1.4, 0xffd788, 0.72);
        for (let i = 0; i < steps; i += 2) {
          const a = i / steps,
            b = Math.min(1, (i + 1) / steps);
          g.lineBetween(
      ...
      g.lineStyle(1.5, 0xffebba, 0.85).strokeCircle(start.x, start.y, 12);
    }
    private drawAtmosphere(t: number, w: number, h: number, moving: boolean, rain: boolean): void {
      const g = this.atmosphere;
      g.clear();
      if (rain) {
        g.lineStyle(1, 0xcde8ee, 0.22);
        for (let i = 0; i < 65; i++) {
          const x = ((((i * 193 - t * 65) % (w + 120)) + w + 120) % (w + 120)) - 60,
            y = ((i * 89 + t * 240) % (h + 80)) - 40;
          g.lineBetween(x, y, x - 12, y + 29);
        }
      }
      // Two distant gulls provide scale, with slow flight rather than a busy particle field.
      for (let i = 0; i < 2; i++) {
        const x = w * (0.2 + i * 0.14) + (moving ? Math.sin(t * 0.13 + i) * 24 : 0),
          y = h * (0.18 + i * 0.045);
        g.lineStyle(1.7, 0x152b35, 0.55)
          .beginPath()
          .moveTo(x - 9, y + 1)
          .lineTo(x - 4, y - 3)
          .lineTo(x, y)
