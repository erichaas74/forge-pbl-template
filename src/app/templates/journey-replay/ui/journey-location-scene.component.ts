...
export class JourneyLocationSceneComponent {
  ...
  resolved(eventId: string): boolean {
    return (
      this.node()
        .events.find((event) => event.id === eventId)
        ?.choices.some((choice) =>
          this.selections().some((selected) => selected.id === choice.id),
        ) ?? false
    );
  }
  constructor() {
    effect((onCleanup) => {
      ...
      let disposed = false,
        handle: JourneyWorldHandle | undefined;
      const scheduled = untracked(() =>
        afterNextRender(
          () => {
            ...
            void this.loader()
              ...
              .catch(() => {
                if (!disposed) this.worldStatus.set('error');
              });
          },
  ...
  symbol(icon: string): string {
    return (
      ({ compass: '⌖', cargo: '▥', sail: '⚑', shore: '⇄', log: '▤' } as Record<string, string>)[
        icon
      ] ?? '•'
    );
  }
