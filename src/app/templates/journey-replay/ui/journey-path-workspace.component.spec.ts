...
describe('branching journey workspace', () => {
  async function setup(session = 1) {
    ...
    await TestBed.configureTestingModule({imports:[JourneyPathWorkspaceComponent],providers:[
      {provide:ActivatedRoute,useValue:{queryParamMap:params}}, {provide:Router,useValue:{navigate}},
    ...
    return {fixture, component:fixture.componentInstance, params, navigate, element:fixture.nativeElement as HTMLElement};
  }
  beforeAll(() => {
  ...
  it('renders the correct main activity for every directly entered session without recording work', async () => {
    ...
    for (let session=1;session<=8;session++) {
      params.next(convertToParamMap({lesson:String(session)})); fixture.detectChanges();
      expect(element.querySelector(session%2 ? 'app-living-journey-map':'app-journey-location-scene')).not.toBeNull();
      ...
      expect(element.querySelector('.activity')?.getAttribute('data-session')).toBe(String(session));
    }
  });
  it('places collapsible tasks directly before AI Tutor and keeps options in that area', async () => {
    const {element}=await setup(2);
    expect([...element.querySelectorAll('.planning-column > details')].map(item=>item.className)).toEqual(['weekly-tasks','tutor']);
  ...
  it('selects a route, enters that landing, and returns to its next map', async () => {
    ...
    (element.querySelectorAll('.choice')[1] as HTMLButtonElement).click(); fixture.detectChanges();
    (element.querySelector('.continue') as HTMLButtonElement).click(); fixture.detectChanges();
  ...
  it('shows all alternate scenes with isolated consequences and exits preview on shared navigation', async () => {
    ...
    for (const scene of config.experience!.nodes) { component.previewScene(scene.id); fixture.detectChanges(); expect(component.node().id).toBe(scene.id); }
    component.previewScene('cape-verde-arrival'); fixture.detectChanges();
    const event = component.node().events[0]; component.choose(event.id,event.choices[0]);
    expect(component.resources()['knowledge']).toBe(18);
    component.next(); fixture.detectChanges();
    expect(component.before().resources['knowledge']).toBe(18);
    expect(component.runtime.state()).toEqual(original);
    params.next(convertToParamMap({lesson:'8'})); fixture.detectChanges();
