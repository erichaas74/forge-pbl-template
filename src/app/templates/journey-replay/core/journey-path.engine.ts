...
import type {
  ...
  JourneyPathNode,
  JourneyPathState,
} from '../domain/journey-path.models';
...
export function pathChoices(node: JourneyPathNode, eventId: string): readonly JourneyPathChoice[] {
  return eventId === 'route'
    ? node.choices
    : (node.events.find((event) => event.id === eventId)?.choices ?? []);
}

export function selectedPathChoice(
  node: JourneyPathNode,
  decisions: readonly JourneyPathDecision[],
  eventId: string,
): JourneyPathChoice | undefined {
...
export function resolveJourneyPath(definition: JourneyPathDefinition, state: JourneyPathState) {
  const path: {
    node: JourneyPathNode;
    provisional: boolean;
    choices: readonly JourneyPathChoice[];
  }[] = [];
  ...
  while (node && path.length < 8) {
    ...
    const nextId =
      [...choices].reverse().find((choice) => choice.nextNodeId)?.nextNodeId ??
      current.defaultNextId;
    node = definition.nodes.find((item) => item.id === nextId);
  }
  return path;
}

export function projectJourneyResources(
  config: JourneyProjectConfig,
  state: JourneyPathState,
  beforeSession = 9,
) {
  const choices = resolveJourneyPath(config.experience!, state)
    .filter((entry) => entry.node.session < beforeSession)
    .flatMap((entry) => entry.choices);
  return projectPathChoices(config, choices);
}

export function projectPathChoices(
  config: JourneyProjectConfig,
  choices: readonly JourneyPathChoice[],
) {
...
export function availablePathChoices(
  node: JourneyPathNode,
  tags: ReadonlySet<string>,
): readonly JourneyPathChoice[] {
...
export function applyJourneyPathEvent(
  config: JourneyProjectConfig,
  state: JourneyPathState,
  event: RuntimeEvent,
): JourneyPathState {
  if (
    ...
    event.payload?.['projectVersion'] !== config.projectVersion
  )
    throw new Error('JOURNEY_PATH_EVENT_INVALID');
  if (
    state.history.some(
      (item) =>
        item.id === event.id || (event.clientEventId && item.clientEventId === event.clientEventId),
    )
  )
    return state;
  ...
  const choice =
    node && pathChoices(node, eventId).find((item) => item.id === event.payload?.['choiceId']);
  ...
  if (
    node.kind === 'map' &&
    !availablePathChoices(node, projectJourneyResources(config, state, node.session).tags).some(
  ...
  const retained = state.decisions.filter((item) => {
    ...
    return (
      entry &&
      entry.session <= node.session &&
      !(item.nodeId === node.id && item.eventId === eventId)
    );
  });
  return {
    ...state,
    ...
    previousPaths: state.decisions.some((item) => !retained.includes(item))
      ? [...state.previousPaths, state.decisions].slice(-20)
      : state.previousPaths,
  };
}
...
export function restoreJourneyPath(config: JourneyProjectConfig, value: unknown): JourneyPathState {
  ...
  if (
    ...
    record.projectVersion !== config.projectVersion ||
    !Number.isSafeInteger(record.revision) ||
    record.revision! < 0 ||
    !Array.isArray(record.decisions) ||
    record.decisions.length > 32 ||
    !Array.isArray(record.history) ||
    record.history.length > 200 ||
    !Array.isArray(record.previousPaths) ||
    record.previousPaths.length > 20
  )
    throw new Error('JOURNEY_PATH_CACHE_INVALID');
  const validDecisions = (decisions: unknown): decisions is JourneyPathDecision[] =>
    Array.isArray(decisions) &&
    decisions.length <= 32 &&
    decisions.every((decision: JourneyPathDecision) => {
      if (!decision || typeof decision !== 'object') return false;
      const node = config.experience!.nodes.find((item) => item.id === decision.nodeId);
      return (
        node &&
        pathChoices(node, decision.eventId).some((choice) => choice.id === decision.choiceId)
      );
    }) &&
    new Set(decisions.map((item) => `${item.nodeId}:${item.eventId}`)).size === decisions.length;
  if (
    !validDecisions(record.decisions) ||
    !record.previousPaths.every(validDecisions) ||
  ...
  if (
    state.decisions.some((decision) => !path.some((entry) => entry.node.id === decision.nodeId)) ||
    path.some(
      (entry) =>
        entry.node.kind === 'map' &&
        entry.choices.some(
