...
let sequence = 0;
const action = (nodeId: string, eventId: string, choiceId: string): RuntimeEvent => ({
  id: String(++sequence),
  clientEventId: String(sequence),
  ...
  sourceId: nodeId,
  payload: { projectVersion: config.projectVersion, eventId, choiceId },
});
const choose = (state: JourneyPathState, nodeId: string, eventId: string, choiceId: string) =>
  applyJourneyPathEvent(config, state, action(nodeId, eventId, choiceId));
...
describe('branching journey paths', () => {
  ...
  it('opens all eight fresh sessions without inventing decisions or resource costs', () => {
    ...
    expect(path.map((entry) => entry.node.kind)).toEqual([
      'map',
      'location',
      'map',
      'location',
  ...
  it('records an alternate arrival and carries its investigation into the next session', () => {
    ...
    const state = choose(
      emptyJourneyPath(config.projectVersion),
      'departure-chart',
      'route',
      choice.id,
    );
  ...
  it('reveals a survey route from a bearing and rejects it before that discovery', () => {
    ...
    expect(
      availablePathChoices(chart, projectJourneyResources(config, state, 3).tags),
    ).toHaveLength(3);
  ...
  it('preserves the other event when revising a choice at the same landing', () => {
    ...
    expect(state.decisions.map((item) => item.choiceId)).toEqual([
      'azores-survey',
      'azores-timber',
    ]);
  });
  it('rejects malformed, cross-version and unreachable cached decisions', () => {
    const state = emptyJourneyPath(config.projectVersion);
    expect(() => restoreJourneyPath(config, { ...state, projectVersion: '1.3.0' })).toThrow(
      'CACHE_INVALID',
    );
    expect(() => restoreJourneyPath(config, { ...state, decisions: [null] })).toThrow(
      'CACHE_INVALID',
    );
    expect(() =>
      restoreJourneyPath(config, {
        ...state,
        decisions: [
          {
            nodeId: 'cape-verde-arrival',
            eventId: 'cape-verde-hold',
            choiceId: 'cape-verde-water',
          },
        ],
      }),
    ).toThrow('CACHE_INVALID');
  });
  it('works with a different project identity without project-name conditions', () => {
    const other = { ...config, projectId: 'navigation-lab' };
    const event = {
      ...action('departure-chart', 'route', node('departure-chart').choices[1].id),
      projectId: other.projectId,
    };
    expect(
      resolveJourneyPath(
        definition,
        applyJourneyPathEvent(other, emptyJourneyPath(other.projectVersion), event),
      )[1].node.id,
    ).toBe('cape-verde-arrival');
  });
  it('rejects broken route references, source references and graph edges', () => {
    for (const mutation of [
      ...
      {
        ...definition,
        nodes: definition.nodes.map((item, i) =>
          i === 0 ? { ...item, defaultNextId: item.id } : item,
        ),
      },
      {
        ...definition,
        nodes: definition.nodes.map((item, i) =>
          i === 0 ? { ...item, choices: [{ ...item.choices[0], routeId: 'missing' }] } : item,
        ),
      },
      {
        ...definition,
        nodes: definition.nodes.map((item, i) =>
          i === 1 ? { ...item, events: [{ ...item.events[0], evidenceIds: ['missing'] }] } : item,
        ),
      },
    ])
      expect(
        validateJourneyPaths(mutation, config.map, config.resources, config.evidence).length,
      ).toBeGreaterThan(0);
  });
});
