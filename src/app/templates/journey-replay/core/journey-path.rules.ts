...
conditions.register({
  id: 'journey.discoveryPresent', version: '1.0.0', status: 'core',
...
export function revealedJourneyChoices(node: JourneyPathNode, tags: ReadonlySet<string>): ReadonlySet<string> {
  let rules = cache.get(node);
  if (!rules) {
    rules = node.choices.map(choice => ({
      id: choice.id, schemaVersion: '1.0', enabled: true, repeatable: true, priority: 0, actions: [],
      ...(choice.requiresAny?.length ? { conditions: { operator: 'OR' as const, conditions: choice.requiresAny.map(tag => ({ type: 'journey.discoveryPresent', targetId: tag })) } } : {}),
    }));
    cache.set(node, rules);
  }
  const scope = { tenantId: 'projection', projectId: 'journey-path-query', projectVersion: '1.0', scopeType: 'student' as const };
  const state: CoreRuntimeState = { ...scope, scope, version: 0, lastUpdated: '', stateValues: Object.fromEntries([...tags].map(tag => [tag, true])), firedRuleIds: [] };
