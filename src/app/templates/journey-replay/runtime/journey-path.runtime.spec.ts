...
describe('local journey practice persistence', () => {
  const scope = {tenantId:'tenant',classId:'class',studentId:'one',projectId:config.projectId,projectVersion:config.projectVersion};
  it('isolates records by tenant, class, student and project version', () => {
    ...
    for (const field of ['tenantId','classId','studentId','projectId','projectVersion']) expect(adapter.load({...scope,[field]:'different'})).toBeUndefined();
  });
  function setup(persistence: {load:()=>unknown;save:ReturnType<typeof vi.fn>}) {
    TestBed.configureTestingModule({providers:[JourneyPathRuntime,{provide:JOURNEY_REPLAY_CONFIG,useValue:config},{provide:JOURNEY_REPLAY_ENROLLMENT,useValue:scope},{provide:JOURNEY_PATH_PERSISTENCE,useValue:persistence}]});
  ...
  it('keeps unsaved choices in memory and retries without charging twice', () => {
    const persistence = {load:()=>undefined,save:vi.fn().mockImplementationOnce(()=>{throw new Error('quota');})};
    const runtime = setup(persistence);
    expect(runtime.choose('departure-chart','route',config.experience!.nodes[0].choices[0].id)).toBe(true);
    expect(runtime.saved()).toBe(false);
    runtime.retrySave();
