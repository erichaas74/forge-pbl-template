...
export class JourneyPathRuntime {
  ...
  private load() {
    try {
      ...
      return value
        ? restoreJourneyPath(this.config, value)
        : emptyJourneyPath(this.config.projectVersion);
    } catch {
      this.loadFailed = true;
      this.error.set(
        'Saved practice could not be read. It has been left intact. New choices stay in this session; reload to retry reading the saved record.',
      );
