...
import type {
  JourneyPathPersistence,
  JourneyPathScope,
  JourneyPathState,
} from '../domain/journey-path.models';

const key = (scope: JourneyPathScope) =>
  `forge:journey-path:1:${JSON.stringify([scope.tenantId, scope.classId, scope.studentId, scope.projectId, scope.projectVersion])}`;
...
export const JOURNEY_PATH_PERSISTENCE = new InjectionToken<JourneyPathPersistence>(
