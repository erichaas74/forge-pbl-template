...
describe('3D workshop interaction and lifecycle', () => {
  ...
  it('keeps the complete lock visible, accepts keyboard placement, pauses release and disposes the renderer', () => {
    ...
    const scroller = host.querySelector<HTMLElement>('.b3d-scene-scroll')!;
    Object.defineProperties(scroller, {
      scrollWidth: { value: 735 },
      clientWidth: { value: 360 },
    });
    expect(ready).toHaveBeenCalledOnce();
    expect(host.querySelector('[data-view]')).toBeNull();
    expect(host.querySelector('[data-mechanism-label]')!.textContent).toBe('ONE LOCK · 3 SCALES');
    expect(host.querySelectorAll('canvas')).toHaveLength(1);
    expect(host.querySelectorAll('.b3d-weight')).toHaveLength(5);
    expect(host.querySelector('[data-place="1"]')).toBeNull();
    expect(host.querySelector('[data-place="2"]')!.textContent).toBe('Place on weight pan');
    Object.assign(root, {
      setPointerCapture() {},
      hasPointerCapture() {
        return false;
    ...
    tick();
    expect(host.querySelector('[data-focus-label]')!.textContent).toBe('WORKING ON SCALE 3 / 3');
    ...
    for (const index of [1, 2, 3]) {
      const scale = scene.getObjectByName(`scale-${index}`)!;
      expect(scale.visible).toBe(true);
      expect(
        scale.children.some((child) => child.userData['weight'] !== undefined && child.visible),
      ).toBe(true);
    }
    expect(scene.getObjectByName('shared-sliding-bolt')).toBeDefined();
