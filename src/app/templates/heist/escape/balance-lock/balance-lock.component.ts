...
@Component({
  ...
  host: { '[class.week-preview]': 'authoringPreview()', '[class.expanded-workshop]': 'expanded()' },
...
export class BalanceLockComponent implements AfterViewInit {
  ...
  readonly piston = computed(() => usesPiston(this.definition()));
  ...
  @ViewChild('expandButton', { static: true }) private expandButton!: ElementRef<HTMLButtonElement>;
  ...
  place(index: number, side: BalanceSide): void {
    if (
      this.paused() ||
      this.locked() ||
      !canPlaceBalanceWeight(this.definition(), side) ||
      !this.blocks().some((b) => b.index === index)
    )
      return;
    this.edited = true;
    ...
    this.notice.set(
      side === 0
        ...
        : this.piston()
          ? 'Weight placed. The pan and piston move in opposite directions.'
  ...
  workshopKey(event: KeyboardEvent): void {
    if (!this.expanded()) return;
    if (event.key === 'Escape') {
