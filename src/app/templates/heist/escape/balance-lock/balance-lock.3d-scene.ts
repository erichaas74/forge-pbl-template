...
import { createBalanceEnvironment } from './balance-lock.environment';
import {
  formatPiece,
  balancePlacements,
...
export function mountBalanceScene(
  ...
  function resize() {
    const width = Math.max(200, viewport.clientWidth),
      ...
      aspect = width / height;
    const worldHeight = Math.max(7.4, (stage.width + 2.0) / aspect);
  ...
  function update(s: BalanceView) {
    ...
    text(
      ...
      target.aligned
        ? piston
          ? 'Pan and piston match. Cutout aligned.'
          : 'This pin is aligned.'
        : piston
          ? target.offset > 0
            ? `Piston mass ${pistonMassLabel(lock, active)}. Add weight to lift it.`
            : `Piston mass ${pistonMassLabel(lock, active)}. Remove weight to lower it.`
          : target.offset < 0
  ...
  function positionWeights(s: BalanceView) {
    ...
    weights.forEach((scaleWeights, scaleIndex) => {
      const bySide = [1, 2].map((side) =>
        scaleWeights.filter((weight) => (weight.fixed || positions[weight.index]) === side),
      );
      scaleWeights.forEach((weight) => {
        ...
        const list = bySide[side - 1],
          ...
          pan = stage.scales[scaleIndex].pans.find((pan) => pan.userData['side'] === side)!;
    ...
    if (raycaster.ray.intersectPlane(new T.Plane(new T.Vector3(0, 0, 1), -0.1), point)) {
      ...
      for (const [index, pan] of scale.pans.entries())
        if (
          ...
          return pan.userData['side'] as BalanceSide;
    }
    return undefined;
  }
  ...
  function click(event: MouseEvent) {
    ...
    if (
      ...
      !s.paused &&
      !s.completed &&
      canPlaceBalanceWeight(lock, Number(placeButton.dataset['place']))
    ) {
      callbacks.place(s.selected, Number(placeButton.dataset['place']) as BalanceSide);
      root.querySelector<HTMLButtonElement>(`[data-weight="${s.selected}"]`)?.focus({ preventScroll: true });
    }
