...
import { dioramaLabel } from '../diorama-label';
...
export function createBridgeDiorama(
  art: BalanceMetalwork,
  d: MachineDefinition,
  labels = true,
): BridgeDiorama {
  const label = (
    ...args: Parameters<typeof dioramaLabel> extends [BalanceMetalwork, ...infer Rest]
      ? Rest
      : never
  ) => {
  ...
  const root = new T.Group(),
    scenery = new T.Group(),
    coordinate = new T.Group(),
    cable = new T.Group();
  root.add(scenery, coordinate, cable);
  const surfaces = new DioramaSurfaces(art),
    stone = surfaces.stone(0x778c8c),
    edge = surfaces.stone(0x445e60),
    wood = surfaces.wood(0x947444),
    grass = art.material({ color: 0x627a45, roughness: 0.95 }),
    ...
    gridInk = art.material({ color: 0x577b7b, roughness: 1 });
  ...
  // Layered wooded ridges, with softer distant colors, frame the river valley.
  for (let layer = 0; layer < 3; layer++) {
    ...
    const ridge = art.mesh(
      ...
      art.material({ color: [0x3f6662, 0x537777, 0x729094][layer], roughness: 1 }),
      scenery,
      0,
      0,
      -4 - layer * 2,
  ...
  for (let i = 0; i < 14; i++) {
    const x = -14 + i * 2.15,
      y = 2.5 + Math.sin(x * 0.29) * 1.35;
    art.cylinder(scenery, x, y - 0.1, -3.7, 0.055, 1.4, wood);
    for (let j = 0; j < 3; j++) {
      const tree = art.mesh(
        new T.ConeGeometry(0.48 - j * 0.1, 0.85, 7),
        art.material({ color: i % 2 ? 0x244a42 : 0x355d4c, roughness: 1 }),
        scenery,
        x,
        y + j * 0.4,
        -3.7,
  ...
  art.box(
    ...
    9.2,
    0.15,
    art.material({ color: 0x183940, roughness: 0.96 }),
  );
  ...
  label(scenery, 'ANCHOR', -9.35, 2.25, 0.54, 1.0, 0.42);
  ...
  const xWheel = wheel(-11, 2.36),
    yWheel = wheel(-11.92, 3.1);
  ...
  const routeLine = (a: readonly number[], b: readonly number[]) => {
    for (let j = 0; j < 12; j++) {
      const start = vector(a).lerp(vector(b), j / 12),
        end = vector(a).lerp(vector(b), (j + 0.55) / 12);
  ...
  anchors.slice(1).forEach((p, i) => {
    ...
    if (cableDefinition.mode === 'route') {
      const a = cableDefinition.route[i],
        b = cableDefinition.route[i + 1];
      label(
        cable,
  ...
  if (cableDefinition.mode === 'diagonal') {
    ...
    label(cable, `${Math.abs(q.x - p.x)} u`, (a[0] + b[0]) / 2, a[1] - 0.48, 0.92, 1.0, 0.65);
  ...
  const cableSegments = Array.from({ length: 60 }, () =>
  ...
  const hoists = [1.7, 5.1].map((z) =>
    art.rod(root, new T.Vector3(1.75, 3.0, z), new T.Vector3(9, 0.85, z), 0.045, art.rope),
  );
  ...
  // A joined corner grille exposes the front and the bridge-facing exit together.
  ...
  const trip = art.rod(
    root,
    new T.Vector3(1.9, 0.8, 5.6),
    new T.Vector3(1.12, 1.75, 5.6),
    0.055,
    art.steel,
  );
  art.box(scenery, 1.1, 7.3, 3.55, 0.18, 0.16, 4.0, art.dark);
  ...
  const gateRopes = [
    art.rod(root, new T.Vector3(1.1, 6.05, 3.4), new T.Vector3(1.1, 3.74, 3.4), 0.038),
    art.rod(root, new T.Vector3(1.1, 6.05, 3.7), new T.Vector3(1.1, 4.9, 3.7), 0.038),
  ];
  const rabbits = Array.from({ length: d.presentation?.rabbits ?? 6 }, (_, i) => {
    const rabbit = createRabbit(
      art,
      art.material({ color: [0xc9b89a, 0xe2d5bc, 0x8c877f][i % 3], roughness: 0.95 }),
      i,
    );
...
export function positionBridgeDiorama(
  art: BalanceMetalwork,
  model: BridgeDiorama,
  d: MachineDefinition,
  ...
  const [coordinate, cable] = bridgeParts(d),
    a = answers[0],
    b = answers[1];
  const position = preview ?? (a.kind === 'coordinate' ? a : { x: 0, y: 0 }),
    p = carriagePoint(coordinate, position);
  ...
  model.lamps.forEach(
    (lamp, i) =>
      (lamp.material = machineReading(d.stages[i], answers[i]).solved ? art.glow : art.dark),
  );
  ...
  art.positionRod(
    model.gateRopes[0],
    new T.Vector3(1.1, 7.3, 3.4),
    new T.Vector3(1.1, model.gate.position.y + 2.82, 3.4),
  );
  art.positionRod(
    model.gateRopes[1],
    new T.Vector3(1.1, 7.3, 3.7),
    new T.Vector3(1.1, model.counterweight.position.y + 0.43, 3.7),
  );
