...
export function bridgeAnswers(d: MachineDefinition, v: MachineView): readonly StageAnswer[] {
  return d.stages.map((s, i) =>
    i === v.active ? v.answer : (v.stages?.[i] ?? machineRules[s.kind].initial(s)),
  );
}
...
export function fittedCable(
  d: Cable,
  index: number,
): { points: [number, number, number][]; state: 'empty' | 'short' | 'slack' | 'taut' } {
  const chosen = d.cables[index];
  if (!chosen) return { points: [], state: 'empty' };
  const required = cableLength(d),
    ratio = chosen.length / required,
    anchors = cablePoints(d);
  const state =
    Math.abs(chosen.length ** 2 - required ** 2) < 1e-8 ? 'taut' : ratio < 1 ? 'short' : 'slack';
  const lengths = anchors
    .slice(1)
    .map((p, i) => Math.hypot(p[0] - anchors[i][0], p[1] - anchors[i][1]));
  ...
  anchors.slice(1).forEach((end, i) => {
    ...
    const start = anchors[i],
      length = lengths[i],
      endAt = Math.min(1, remaining / Math.max(length, 1e-9));
    for (let j = 1; j <= 20; j++) {
      const t = (endAt * j) / 20;
      points.push([
        ...
        start[1] +
          (end[1] - start[1]) * t -
          (state === 'slack' ? Math.sin(t * Math.PI) * Math.min(1.5, (ratio - 1) * 3) : 0),
        0.98,
      ]);
    }
    remaining -= length;
...
export function bridgeRelease(time: number) {
  return {
    pin: ease(time / 0.65),
    deck: ease((time - 0.75) / 2.15),
    gate: ease((time - 3.1) / 1.1),
    bolt: ease((time - 2.9) / 0.3),
  };
}
export function crossingRabbit(index: number, time: number, idle: number) {
  const elapsed = time - 4.5 - index * 0.48,
    p = clamp(elapsed / 6.6);
  const hop = p > 0 && p < 1 ? Math.max(0, Math.sin(p * Math.PI * 18)) : 0;
  const lane = 2.7 + (index % 3) * 0.95,
    landing = 2.7 + Math.floor(index / 3) * 1.75;
  return {
    x: -1.05 + (11.45 + (index % 3) * 1.3 + Math.floor(index / 3) * 0.4) * p,
    y: 0.93 + hop * 0.23,
    z: lane + (landing - lane) * ease((p - 0.87) / 0.13),
...
export class BridgeSequence {
  ...
  constructor(
    private readonly d: MachineDefinition,
    view: MachineView,
  ) {
  ...
  update(v: MachineView, dt: number, settled: boolean) {
    const answers = bridgeAnswers(this.d, v),
      keys = answers.map((a) => JSON.stringify(a));
    ...
    const all = bridgeSolved(this.d, answers),
      solved = machineReading(this.d.stages[v.active], v.answer).solved;
    let engage = false,
      finished = false;
