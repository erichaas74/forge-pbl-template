...
import { bridgeAnswers, bridgeParts, bridgeRelease, bridgeSolved, BridgeSequence, BRIDGE_DURATION,
  cablePoints, carriageInput, carriagePoint, crossingRabbit, fittedCable } from './bridge-cage.motion';
import { createBridgeDiorama, positionBridgeDiorama } from './bridge-cage.model';

const step = requireEscapeMission(data).steps[5], puzzle = step.puzzle;
if (puzzle.type !== 'machine-lock') throw Error('Expected bridge');
const d = puzzle.lock, initial = initialMachine(d).stages, solved = d.stages.map(machineWitness);
const view = (stages: readonly StageAnswer[] = initial, active = 0): MachineView => ({
  stages, active, answer: stages[active], freelySelectStages: true, selected: null, testing: false,
  trial: 0, passed: false, completed: false, paused: false, reducedMotion: false,
});
...
describe('Bridge holding-enclosure workshop', () => {
  it('preserves all four grade goals, exact cable lengths, and bounded rail geometry', () => {
    const targets = [{x:6,y:4},{x:2,y:-3},{x:3,y:2},{x:2,y:5}], lengths = [6,8,7.5,10];
    [step.puzzle, ...Object.values(step.gradePuzzles ?? {})].forEach((p, i) => {
      ...
      expect(coordinateTarget(coordinate)).toEqual(targets[i]); expect(cableLength(cable)).toBe(lengths[i]);
      ...
      expect(carriageInput(coordinate, -100, 100)).toEqual({x:coordinate.min,y:coordinate.max});
    });
  });
  it('shows a proportional shortfall, a slack curve, and the exact routed or diagonal span', () => {
    for (const p of [step.puzzle, ...Object.values(step.gradePuzzles ?? {})]) {
      ...
      const [, cable] = bridgeParts(p.lock), anchors = cablePoints(cable);
      const short = fittedCable(cable, 0), taut = fittedCable(cable, 2), slack = fittedCable(cable, 4);
      expect(fittedCable(cable, -1).state).toBe('empty');
      expect(short.state).toBe('short'); expect(taut.state).toBe('taut'); expect(slack.state).toBe('slack');
      expect(short.points.at(-1)?.slice(0,2)).not.toEqual(anchors.at(-1)?.slice(0,2));
      expect(taut.points.at(-1)?.slice(0,2)).toEqual(anchors.at(-1)?.slice(0,2));
  ...
  it('auto-engages a newly committed correct stage once, never a drag or a stage switch', () => {
    ...
    const partial = [solved[0], initial[1]], v = view(partial);
    ...
    expect(sequence.update({...v,testing:true,passed:true,trial:1},1,true).finished).toBe(true);
  ...
  it('keeps the bridge raised if only the cable fits, including direct entry to stage two', () => {
    const partial = [initial[0],solved[1]], sequence = new BridgeSequence(d,view(initial,1));
    expect(sequence.update(view(partial,1),0.4,true)).toMatchObject({engage:true,all:false});
    expect(bridgeSolved(d,partial)).toBe(false);
    expect(bridgeAnswers(d,{...view(),stages:undefined})).toEqual(initial);
    const wrong = view();
    expect(sequence.update({...wrong,testing:true,passed:false,trial:1},1,true).finished).toBe(true);
  });
  it('restores solved poses, pauses hidden frames, and replays reduced motion without auto trials', () => {
    const v = view(solved,1), sequence = new BridgeSequence(d,v);
    expect(sequence.time).toBe(BRIDGE_DURATION); expect(sequence.update(v,1,true).engage).toBe(false);
    const replay = {...v,testing:true,passed:true,trial:1};
    sequence.update(replay,0.5,true); const t=sequence.time;
    sequence.update({...replay,paused:true},2,true); expect(sequence.time).toBe(t);
    sequence.update(replay,0,true); expect(sequence.time).toBe(t);
  ...
  it('fully lowers the deck and clears the gate before any rabbit enters the crossing', () => {
    expect(bridgeRelease(2.9).deck).toBe(1); expect(bridgeRelease(2.9).gate).toBe(0);
    ...
    for (let i=0;i<6;i++) {
      ...
      for (let time=0;time<14;time+=0.05) {
        ...
        if (rabbit.x > 1.1) {
          expect(bridgeRelease(time).gate).toBe(1); expect(bridgeRelease(time).deck).toBe(1);
          expect(rabbit.y).toBeGreaterThanOrEqual(0.93); expect(rabbit.z).toBeGreaterThan(1.7); expect(rabbit.z).toBeLessThan(5.1);
        }
      }
  ...
  it('poses the actual scene and owns its geometry/material disposal', () => {
    const art = new BalanceMetalwork(false), model = createBridgeDiorama(art,d);
    const before = positionBridgeDiorama(art,model,d,initial,0,0,0);
    expect(before.release.deck).toBe(0); expect(model.coordinate.visible).toBe(true);
    const final = positionBridgeDiorama(art,model,d,solved,1,BRIDGE_DURATION,0);
    expect(final.escaped).toBe(6); expect(model.deck.rotation.z).toBe(0); expect(model.gate.position.y).toBeGreaterThan(4);
    expect(model.cable.visible).toBe(true); expect(model.coordinate.visible).toBe(false);
    expect(model.rabbits.every(r=>r.root.position.x>9)).toBe(true);
    const geometry = [...art.geometries][0], texture=[...art.textures][0], dispose=vi.spyOn(geometry,'dispose'), disposeTexture=vi.spyOn(texture,'dispose');
    const box=new T.Box3().setFromObject(model.deck); expect(box.max.x).toBeCloseTo(9,0);
    art.dispose(); expect(dispose).toHaveBeenCalledOnce();expect(disposeTexture).toHaveBeenCalledOnce();expect(art.geometries.size).toBe(0);
  });
  it('accepts only a presentation upgrade and rejects changed geometry or malformed stage pairs', () => {
    ...
    expect(acceptsBridgeCageUpgrade(JSON.stringify(old),JSON.stringify({...d,stages:[{...d.stages[0],max:9},d.stages[1]]}))).toBe(false);
    expect(()=>validateMachine({...d,presentation:{kind:'bridge-cage',rabbits:7}})).toThrow();
