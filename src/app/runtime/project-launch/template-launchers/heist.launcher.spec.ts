...
import restoration from '../../../../../public/projects/shadow-gallery/versions/2.0.0/project.json';
...
describe('Heist experience launch routing', () => {
  it('loads the gallery, animal escape and previous guided experience', async () => {
    const current = await heistLauncher.load(request('shadow-gallery', restoration)); expect(current.component.name).toContain('Restoration'); expect(current.integratedHeader).toBe(true);
    const galleryRequest = request('shadow-gallery', gallery);
    const next = await heistLauncher.load({ ...galleryRequest, project: { ...galleryRequest.project, projectVersion: gallery.projectVersion }, session: createLocalPreviewSession('shadow-gallery', gallery.projectVersion) }); expect(next.component.name).toContain('Gallery'); expect(next.integratedHeader).toBe(true);
  ...
  it('rejects mismatched gallery packages and unsupported official authority', async () => {
    await expect(heistLauncher.load(request('shadow-gallery', { ...gallery, projectVersion: '0.0.0' }))).rejects.toThrow('PROJECT_ID_MISMATCH');
    await expect(heistLauncher.load(request('shadow-gallery', { ...restoration, projectVersion: '0.0.0' }))).rejects.toThrow('PROJECT_ID_MISMATCH');
