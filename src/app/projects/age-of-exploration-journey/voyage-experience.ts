import type { JourneyPathChoice, JourneyPathDefinition, JourneyPathEvent, JourneyPathNode } from '../../templates/journey-replay/domain/journey-path.models';
import type { JourneyMapConfig, JourneyProjectConfig, JourneyRouteDefinition } from '../../templates/journey-replay/domain/journey-replay.models';

type Option = Omit<JourneyPathChoice, 'id'> & { readonly id: string };
const option = (id: string, label: string, summary: string, consequence: string, nextTask: string, learning: string, resourceChanges: Record<string, number>, grants: string[], effect: JourneyPathChoice['effect']): Option => ({ id, label, summary, consequence, nextTask, learning, resourceChanges, grants, effect });
const event = (id: string, label: string, observation: string, question: string, icon: JourneyPathEvent['object']['icon'], x: number, y: number, evidenceIds: string[], choices: Option[]): JourneyPathEvent => ({ id, label, observation, question, object: { icon, x, y }, evidenceIds, choices });
...
export function withVoyageExperience(base: JourneyProjectConfig): JourneyProjectConfig {
  ...
  function passage(nodeId: string, from: string, to: string, nextNodeId: string, label: string, weeks: number, task: string, requiresAny?: string[]): JourneyPathChoice {
    ...
    const west = Math.min(start.longitude, end.longitude, -25) - (from === 'cape-town' || to === 'cape-town' ? 7 : 0);
    extraRoutes.push({ id, fromLocationId: from, toLocationId: to, coordinates: [start, { longitude: west, latitude: start.latitude * .65 + end.latitude * .35 }, { longitude: west, latitude: start.latitude * .25 + end.latitude * .75 }, end], distanceLabel: `Model passage · ${weeks} weeks`, risk: weeks > 2 ? 'high' : 'moderate', windLabel: requiresAny ? 'Survey passage revealed by your observations' : weeks > 2 ? 'Open water; compare the reserve' : 'Island or coastal refuge' });
    return { id, label, routeId: id, nextNodeId, requiresAny, summary: `${weeks} model weeks · ${weeks * 4} supply points${requiresAny ? ' · newly charted' : ''}`, consequence: `Your ship reaches ${end.shortName}. The route costs ${weeks * 4} supply points and ${weeks} weeks in this classroom model.`, resourceChanges: { supplies: -weeks * 4, time: weeks, crew: weeks > 2 ? -5 : -2, knowledge: requiresAny ? 8 : 2 }, grants: requiresAny ? ['charts'] : [], effect: 'sail', nextTask: task, learning: 'Compare direction, waypoints, time and reserve. These passage costs are simulation rules.' };
  }
  function chart(id: string, session: number, locationId: string, title: string, choices: JourneyPathChoice[]): JourneyPathNode {
    return { id, session, kind: 'map', locationId, title, defaultNextId: choices[0].nextNodeId, tasks: ['Inspect a route and its destination.', 'Compare passage costs with what your earlier choices left aboard.'], product: 'A chosen passage and the investigation it opens.', learning: 'Use geographic evidence and the expedition’s current resources to choose the next investigation.', choices, events: [] };
  }
  const nodes: JourneyPathNode[] = [
    chart('departure-chart', 1, 'lisbon', 'Where will you investigate first?', [
      passage('departure-chart', 'lisbon', 'azores', 'azores-arrival', 'The Azores · bearings & cargo', 2, 'Inspect the headlands and choose the last load for the cargo berth.'),
      passage('departure-chart', 'lisbon', 'cape-verde', 'cape-verde-arrival', 'Cape Verde · wind & reserves', 3, 'Inspect the crosswind, then prepare your ship for the next passage.'),
    ]),
    ...
    chart('azores-chart', 3, 'azores', 'A coast, a market, or a survey?', [
      passage('azores-chart', 'azores', 'cape-verde', 'storm-roadstead', 'Cape Verde · damaged sail', 2, 'Inspect the torn canvas and compare repair with shelter.'),
      passage('azores-chart', 'azores', 'elmina', 'market-arrival', 'Elmina · exchange accounts', 3, 'Compare the shore accounts before deciding how to resupply.'),
    ...
    chart('cape-verde-chart', 3, 'cape-verde', 'Follow the coast or find a refuge?', [
      passage('cape-verde-chart', 'cape-verde', 'elmina', 'market-arrival', 'Elmina · negotiate resupply', 2, 'Keep the ship’s needs and the households’ reserve visible in the exchange.'),
      passage('cape-verde-chart', 'cape-verde', 'azores', 'refuge-arrival', 'Azores · repair & bearings', 3, 'Inspect the sail at refuge and check the headland bearings.'),
  ...
  for (const [id, from] of [['storm-chart','cape-verde'],['market-chart','elmina'],['refuge-chart','azores']] as const) {
    nodes.push(chart(id, 5, from, 'An ocean crossing or a return?', [
      passage(id, from, 'recife', 'brazil-arrival', 'Brazilian coast · two perspectives', 3, 'Compare two reconstructed accounts and decide how your crew will approach the shore.'),
      passage(id, from, 'cape-town', 'cape-arrival', 'The Cape · repair & navigation', 4, 'Inspect the sail and the coastline; compare continuing with taking shelter.'),
      passage(id, from, 'lisbon', 'return-arrival', 'Lisbon · sheltered return', 2, 'Compare what the shorter return preserves with what the voyage leaves uncertain.', ['shelter']),
    ]));
  }
  nodes.push(
  ...
  for (const [id, from] of [['brazil-chart','recife'],['cape-chart','cape-town'],['home-chart','lisbon']] as const) {
    const harbor = from === 'lisbon' ? 'cape-verde' : 'lisbon';
    const harborNode = from === 'lisbon' ? 'final-harbor' : 'final-home';
    nodes.push(chart(id, 7, from, 'Which evidence will close your voyage?', [
      passage(id, from, harbor, harborNode, from === 'lisbon' ? 'Cape Verde · revisit the accounts' : 'Lisbon · compare the voyage record', 3, 'Select one recorded passage and connect it to its observed consequence.'),
      passage(id, from, 'azores', 'final-atlas', 'Azores · check the chart', 3, 'Compare the route record with two headland observations.'),
      passage(id, from, 'azores', 'final-atlas', 'Follow your survey observations', 2, 'Use the checked observations from your earlier investigation to examine a shorter modeled return.', ['charts']),
    ]));
  }
  nodes.push(
    site('final-home', 8, 'lisbon', 'The record comes ashore', 'home', undefined, ['Inspect the complete route and its remaining resources.', 'Choose the evidence that explains a decision and a limit.'], 'An account is stronger when its route, sources and consequences can be inspected together.', [record('final-home', true), exchange('final-home')]),
    site('final-atlas', 8, 'azores', 'A chart worth questioning', 'island', undefined, ['Check the headlands against the route you recorded.', 'Assemble the evidence for the voyage’s strongest remaining question.'], 'New observations can confirm or challenge a chart; retain what remains uncertain.', [bearings('final-atlas'), record('final-atlas', true)]),
    site('final-harbor', 8, 'cape-verde', 'Return to the roadstead', 'harbor', undefined, ['Compare the new approach with the earlier wind observation.', 'Assemble both source perspectives beside the route.'], 'Returning to a place creates a comparison, not proof that the first interpretation was complete.', [bearings('final-harbor', true), record('final-harbor', true)]),
  );
  const experience: JourneyPathDefinition = { schemaVersion: '1.0', capability: 'branchingJourney', startNodeId: 'departure-chart', nodes };
  const map: JourneyMapConfig = { ...base.map, routes: [...base.map.routes, ...extraRoutes] };
  return { ...base, projectVersion: '1.4.0', subtitle: 'A branching Atlantic investigation', map, experience };
}
