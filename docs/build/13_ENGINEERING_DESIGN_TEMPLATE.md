...
All learning steps and final views now use one header, composed through optional `DESIGN_CHROME` toolbar/guide templates supplied by the installed renderer. Directions, samples and notebooks open on request. Optional launcher/sample `integratedHeader` hints suppress duplicate host presentation while preserving navigation and teacher-guide access. The same-origin toolbar protocol forwards validated actions to existing simulation controls; it does not change persistence or solar calculations. See [the layout, contracts and verification](../engineering-design/SIMPLIFIED_LAB_LAYOUT.md).
