...
`BlockDesign` contains up to 100 measured blocks and 12 labelled point targets, with optional cylindrical openings and a central display object. Units are metres. +X is east, +Y is up, +Z is south. Y is the block base, not its centre. Rotation follows the Three.js Y-axis convention. The UI offers rotation in degrees, positions, dimensions in centimetres, stacking, selection, removal, and undo. Validation rejects non-finite dimensions, duplicate IDs, out-of-range fields, overlapping solids and openings without a sufficient rim. Touching faces are allowed. Structural stability/gravity is not modelled.
...
Solar angles come from SunCalc 1.9.0. Analytic parallel-ray tracing now determines direct light at target centres and rendered surface points: the ray must clear every solid, including the complete depth of a cylindrical bore. Inserts multiply their color transmission; the central object can receive and block light. This replaces the earlier opaque convex-hull ground polygons, while retaining that helper for geometric regression tests. The renderer modulates Three's direct lighting with the same solid equations instead of a shadow map. At or below zero geometric altitude there is no direct sunlight. Very long shadows may extend outside the chosen camera view; target calculations are not clipped by the camera.
...
New trial settings include `utcInstant` and `modelVersion: solar-optics-2.0`; these use the existing settings contract. Older saved trials, including `solar-geometric-1.1` records, remain readable. Learning guides support model exploration before writing. See [the inspected-source integration plan](../engineering-design/SUN_MODEL_PLAN.md) and [optics implementation](../engineering-design/OPTICS_AND_SCULPTURE.md).

Season buttons use US Naval Observatory event instants for **2025–2030**, converted to the selected location's local date. Date inputs are bounded to that range. When Solar noon is selected, season shortcuts recompute noon for each date. Manual time selection keeps the clock-time rule. The original globe remains a location selector; the optional Why seasons? panel adds a synchronized Earth spin/orbit/tilt schematic.
...
This release is Preview. Next phase: calibrate with classroom blocks and outdoor observations, agree on measurement tolerances, then connect shared team persistence and authoritative final submission using the LMS adapters. Optional later additions include photographs, shadow-tip traces and terrain horizons. These are not claimed as implemented.
# Optical design extension

`DesignBlock.aperture` optionally declares one centred cylindrical bore (`axis`, measured `diameter`, `insert`, `color`). `BlockDesign.displayObject` optionally declares a built-in sculpture, measured geometry, position, rotation and finish. Old 1.0 envelopes remain readable; new captures declare `solar-optics-2.0`. Template controls emit ordinary validated design changes and all optical state is included in trial snapshots and blueprint export. The installed simulation evaluates direct transmission and object lighting for both the canvas and final four-date evidence. See [full optical contract and verification notes](../engineering-design/OPTICS_AND_SCULPTURE.md).

## Day demonstration extension

See [Build, Show Sun, and a complete day](../engineering-design/SUN_DAY_AND_LEARNING.md) for the simplified canvas flow, traced optical rays, bounds-based camera, synchronized Earth view, configurable observation rules, nearby-date comparisons, compatible contracts and verification. `DESIGN_VIEW_REQUEST` is optional local navigation. `DesignCheck.settings` permits at most four short string/finite-number parameters; the simulation validates their meaning. Final presentation uses one common camera frame and shows the currently displayed date/time.

## Ordered practice and project learning extension

Optional `EngineeringDesignConfig.learningSequence` defines a validated practice design and up to ten content-driven steps with installed activity IDs, practice/project workspace selection, optional research questions/explanations and guide visibility. Snapshot `practiceDesign` and `learningStepId` are optional, preserving previous designs and old envelopes. Runtime `selectLearningStep` validates and emits `engineering.learningStepSelected`; `saveDesign` accepts an optional workspace and retains the existing validated design event. The optional `DESIGN_CHANGE` simulation callback uses this runtime boundary. Captures carry workspace/step settings, and replay selects the corresponding design workspace before restoring the observation. Generic packages without a sequence receive no new required simulation input.

`DesignTarget.settings` now supports at most eight short string/finite-number observation parameters. The installed solar plugin uses these for fixed shadow-tip calibration marks. Both validators enforce the bound. The plugin cancels pending captures on lesson changes, checks the current editable activity on design requests and protects read-only examples. Curriculum and tool behavior remain outside the generic core.

The Preview's configured order is sundial/day marks, seasonal observation and initial hypothesis, Earth-tilt explanation and revision, sundial equinox/solstice challenge, then the creative monument and its final demonstration. See [the complete sequence, science, files and verification](../engineering-design/SUNDIAL_LEARNING_SEQUENCE.md). This resolves the ordered-practice capability gap without replacing existing drafts or introducing a custom project page.

## Compact workspace presentation

All learning steps and final views use one compact lab toolbar, composed through optional `DESIGN_CHROME` toolbar/guide templates supplied by the installed renderer. The standard project navigation header remains above the lab, with Back to projects, Opening and Final example links. Directions, samples and notebooks open on request. The optional launcher `integratedHeader` hint remains available, but the engineering launcher uses the standard host navigation. The completed sample retains its compact presentation and teacher-guide access. The same-origin toolbar protocol forwards validated actions to existing simulation controls; it does not change persistence or solar calculations. See [the layout, contracts and verification](../engineering-design/SIMPLIFIED_LAB_LAYOUT.md).

## Solar calendar as the final project

The last configured step is **Build a solar calendar**. Its new-design starter and first gallery option supply an eight-gate stone circle with three jeweled cylindrical roof openings, without preplaced targets. The completed example supplies measured seasonal targets and four reference records produced by the existing SolarReview evaluator. Existing drafts are never replaced by a new starter; gallery adoption remains explicit and reversible. This is a refinement of Preview curriculum through the existing contracts, with no runtime/schema/version migration. See [the model, learning challenge, sources and verification](../engineering-design/SOLAR_CALENDAR_FINAL.md).

The installed solar renderer also supports **Calendar sunstones**: direct floor placement, measured/keyboard placement, immutable Sun observation metadata, live light comparison, exact-time revisit and seasonal test setup. The existing eight-field target settings and design/check callbacks support this behavior without new generic contracts. Read-only/final views inspect fixed markers; editable monument mode creates them. See [the marker workflow, files and verification](../engineering-design/CALENDAR_SUNSTONES.md).
