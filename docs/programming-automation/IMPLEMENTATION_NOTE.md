...
The robot activity uses the existing host return bar and one compact project header. The header owns the project title, save status, view and mission selectors, mobile course/code/math selector, and Guide trigger. Separate lab navigation, mission, and mobile navigation bands have been removed. The mission goal stays next to the course; the instructional banners and command reference are now in the shared `TaskGuideComponent` dialog. Recorded examples receive replay-specific guidance. Hints still follow the existing reasoning gate, while observed-run results and the Open reasoning action stay with the course.

This update modifies the lab's TypeScript, HTML, CSS, and component test; the command editor's TypeScript, HTML, and CSS; and this note. No new files or dependencies are needed for this pass. It reuses the host bar, shared guide, existing command graphics and descriptions, and existing runtime actions. No public runtime contract, persistence format, curriculum requirement, specification exception, or template capability gap changed. The current requested layout work is complete.

Validation: the production build and all 23 robot-template tests pass. The component test opens/closes the guide, checks that help does not unlock reasoning, and switches to the portfolio and back while preserving the draft. Desktop/mobile browser checks verify one return bar and one project `h1`, no horizontal overflow, guide focus, Escape dismissal, and focus restoration. Existing style-budget warnings remain elsewhere. `test:architecture` still fails on the two previously documented baseline violations: `core/index.ts` importing `./templates`, and `projects/mystery-substance/lab-kit/render-quality.service.ts` defining a project service; both files match HEAD and were not modified by this update.
