...
The showcase starts automatically, advances chapters every four seconds, and stops at the champion. The storyboard cycles its three recap cards within that four-second chapter. It is a fast pitch preview of the longer 75-second video in `mock-recap-script.md`. Supply the MP4 and enable `recapVideo`/`recapCaptions` as documented in that script to replace the storyboard. Fast autoplay previews four seconds of the video; pause the demo and use the native video controls to watch the full reel. Clips should revisit first attempts, evidence, and corrections, then show qualification seeds and end with a clean transition into the bracket. There is no automatic video editing or classroom-record import in this build. The fast preview uses 450 ms camera travel and 250 ms reveals; ordinary rehearsal retains its configured broadcast timing.
