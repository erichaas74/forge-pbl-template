...
The fictional recap derives qualification points from its three invented event records; those same story moments become the narrator's warm-up. Nova's early delivery-cost mistake returns in the final wager, giving the ending a narrative payoff. The demo currently shows a timed storyboard, not an assembled video. [The video script](../../public/projects/championship-show/mock-recap-script.md) supplies a 75-second shot list, narration, captions, opening frame, and exact instructions for attaching a mock MP4. Fast autoplay previews the video for four seconds and falls back to the storyboard on load failure. Pause the demo and use native video controls to watch the full reel. Real classroom clip selection/assembly is still a future integration.

Three audience games are available during both quiz breaks: Odd One Out (a silly lineup with a comic explanation), Sell Me This! (ridiculous invention pitches), and Reaction Cam (dramatic freeze-frame challenges). Odd One Out replaces the former mystery zoom; it needs no image asset. These games have no competition points or academic scoring and include seated/pass options. Autoplay reveals each gag within its four-second chapter. Week 1 and Week 2 use dedicated cinematic recap images; see [RECAP_ART.md](RECAP_ART.md) for asset paths, exact ImageGen prompts, and optional per-highlight image configuration.

All final-demo navigation now lives in one top header: project return, chapter links, page title, playback, recap-week choices, and game selection. The audience stage hides director controls; duplicate lower navigation, setup links, and production explanations are removed from the pages. **Teacher setup** opens a native dialog and pauses autoplay. It contains the rehearsal setup link, manual cues/cameras, sound/motion controls, video resources, and evidence/recap explanations. Escape/Close returns focus to the teacher button. This is local-demo disclosure, not authenticated role enforcement; the existing authoritative-classroom capability gap still applies. The old optional `mysteryImage` field is accepted for package compatibility.
