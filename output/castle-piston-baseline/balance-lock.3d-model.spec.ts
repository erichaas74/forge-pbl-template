...
describe('3D hanging balance mechanism', () => {
  it('connects every scale to its own hanging pin inside one housing with one bolt', () => {
    ...
    stage.pins.forEach((pin, i) => {
      ...
      const rightPan = stage.scales[i].pans[1];
      expect(pin.rope.position.y + ropeLength).toBeCloseTo(rightPan.position.y - 0.04, 8);
      expect(pin.rope.getWorldPosition(new Vector3()).x).toBeCloseTo(
        rightPan.getWorldPosition(new Vector3()).x,
        8,
      );
      expect(stage.scales[i].root.visible).toBe(true);
    });
    expect(stage.scales[0].beam.rotation.z).toBeCloseTo(0);
    expect(stage.scales[1].beam.rotation.z).toBeGreaterThan(0);
