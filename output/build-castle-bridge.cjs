...
async function main() {
  ...
  const logger = new logging.Logger('castle-bridge');
  logger.subscribe(entry => console.log(entry.message));
  const run = await architect.scheduleTarget({project:'forge-pbl-template',target:'build',configuration:'production'}, {browser:'output/castle-week-bootstrap.ts',tsConfig:'output/castle-week-app-tsconfig.json',outputPath:'output/castle-bridge-code-build',assets:[],deleteOutputPath:true}, {logger});
