...
// src/app/templates/simulation-decision/runtime/simulation-decision.persistence.ts
var BrowserSimulationDecisionPersistenceAdapter = class {
  constructor(storage = safeBrowserStorage(), session) {
    this.session = session;
    this.store = new ScopedBrowserStore(
      "simulation-decision",
      storage,
      (value) => isCompatibleState(value)
    );
  }
  session;
  store;
  ...
  save(state) {
    this.store.save(this.scope(state.projectId, state.projectVersion), state);
  }
...
function isCompatibleState(value) {
  return typeof value === "object" && value !== null && "projectId" in value && typeof value.projectId === "string" && "projectVersion" in value && typeof value.projectVersion === "string" && "ledger" in value && Array.isArray(value.ledger) && "inventory" in value && Array.isArray(value.inventory);
}

// src/app/projects/frontier-trading/frontier-trading.builder-info.ts
var frontierTradingBuilderInfo = {
  ...
  statusSummary: "The guided Grade 5 practice loop and read-only final showcase are implemented. The historically grounded HBC route network and server-authoritative class competition remain planned work.",
  studentPath: [
  ...
  paths: [
    ...
    },
    {
      ...
      purpose: "Published goods, markets, routes, progression, forecast, events, and report settings.",
      kind: "source"
    },
    {
    ...
    },
    {
      ...
      purpose: "Read-only presentation stages, game-record summaries, pitch timer, and peer-question protocol.",
      kind: "source"
    },
    {
      label: "Reusable realistic wagon layer",
      path: "src/app/templates/simulation-decision/ui/art/illustrated-wagon.component.html",
      purpose: "Transparent photographic wagon base with replaceable cargo and lightweight movement layers shared across game screens.",
      kind: "source"
    },
    {
    ...
    },
    {
      ...
      purpose: "Transparent sack, crate, wrapped-bale, and rope-coil sprites stacked from live inventory quantities.",
      kind: "source"
    },
    {
      label: "Wagon cargo art system",
      path: "docs/frontier-trading/WAGON_CARGO_ART_SYSTEM.md",
      purpose: "Package mapping, quantity levels, label rules, accessibility contract, and asset replacement guidance.",
      kind: "document"
    },
    {
      label: "Pre-rendered shop scenes",
      path: "public/frontier-trading/shop-scenes",
      purpose: "Optimized exterior town and four shop-interior backgrounds used under live HTML controls.",
      kind: "source"
    },
    {
      label: "Pre-rendered route map",
      path: "public/frontier-trading/map-scenes",
      purpose: "Painted frontier terrain used beneath live SVG routes, checkpoints, destinations, and journey progress.",
      kind: "source"
    },
    {
      label: "Pre-rendered opening scene",
      path: "public/frontier-trading/setup-scenes",
      purpose: "Painted company-charter background displayed beneath the live transport and setup controls.",
      kind: "source"
    },
    {
    ...
    },
    {
      ...
      purpose: "Map asset, coordinate alignment, route-overlay contract, prompt direction, and replacement rules.",
      kind: "document"
    },
    {
    ...
    },
    {
      ...
      purpose: "Implemented presentation flow, evidence sources, peer protocol, controls, and live-system boundary.",
      kind: "document"
    },
    {
      label: "Live trading system design",
      path: "docs/frontier-trading/LIVE_TRADING_FINAL_DESIGN.md",
      purpose: "Authoritative sessions, reservations, atomic exchanges, live updates, and recovery.",
      kind: "document"
    }
  ],
  changes: [
  ...
  historicalDirection: [
    ...
    "Cree and Assiniboine traders, First Nations route knowledge and commercial agency, and M\xE9tis freight labor must be represented as historical actors, not game modifiers.",
  ...
  guardrails: [
    ...
    "Map backgrounds never contain route lines, labels, checkpoints, math, locks, or game state; those remain live SVG and HTML overlays aligned to the 100 \xD7 80 map coordinates.",
    ...
    "Cargo shape communicates package type, a restrained tag names the material, and \xD7 quantity communicates the exact amount; color alone never carries meaning.",
  ...
  roadmap: [
    {
      phase: "1 \xB7 Guided practice bridge",
      status: "current",
      outcome: "One-action scaffolding, profit forecast, trail math, selling, score, and reflection."
    },
    {
      phase: "2 \xB7 Reviewed HBC scenario package",
      status: "next",
      outcome: "Period-correct route network, pack loads, source cards, requirements, traps, and answer keys.",
      dependsOn: "Social studies and Indigenous content review."
    },
    {
      phase: "3 \xB7 Two-company live trade",
      status: "planned",
      outcome: "Authenticated session, reservation, math gate, two approvals, atomic settlement, and shared receipt.",
      dependsOn: "Reusable live-session and resource-exchange capabilities."
    },
    {
      phase: "4 \xB7 Timed four-company final",
      status: "planned",
      outcome: "Authoritative clock, disruptions, deliveries, score projections, audit, and final evidence.",
      dependsOn: "Two-company retry, conflict, privacy, and reconnect checks passing."
    },
    {
      phase: "5 \xB7 Classroom release",
      status: "blocked",
      outcome: "Accessible, balanced, historically reviewed, load-tested final project competition.",
      dependsOn: "Teacher rehearsal, content review, recovery drill, and four-company pilot."
    }
  ]
};

// src/app/runtime/project-launch/template-launchers/simulation-decision.launcher.ts
var simulationDecisionLauncher = {
  ...
  async load(request) {
    ...
    const providers = [
      ...
      { provide: SIMULATION_DECISION_SESSION_CONTEXT, useValue: request.session },
      {
        ...
        useFactory: () => new BrowserSimulationDecisionPersistenceAdapter(void 0, request.session)
      },
      SimulationDecisionRuntimeService
    ];
    if (projectIntroRegistry.find(request.project.id) || request.project.finalExampleMode === "template") {
      providers.push({
    ...
    if (request.view === "builder-info") {
      const module2 = await import("./chunk-ZTRAGV33.js");
      return {
        component: module2.SimulationDecisionBuilderInfoComponent,
        providers: [
          ...providers,
          {
            provide: SIMULATION_DECISION_BUILDER_INFO,
            useValue: request.project.id === "frontier-trading-company" ? frontierTradingBuilderInfo : defaultBuilderInfo(config, request.project.route)
          },
          { provide: SIMULATION_DECISION_PROJECT_ROUTE, useValue: request.project.route }
        ]
      };
    }
    const module = await import("./chunk-CCRBMRMY.js");
    return {
...
function requireSimulationConfig(value, projectId) {
...
function defaultBuilderInfo(config, studentRoute) {
  return {
...
export {
  simulationDecisionLauncher
};
//# debugId=c184e110-df67-5541-b493-8c88443f03cb
//# sourceMappingURL=chunk-DRZUHS7N.js.map
