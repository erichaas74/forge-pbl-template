(() => {
  ...
  let weeklyPreview = false, centerDayPlayback = false;
  let monumentEditor;
  ...
  function init() {
    ...
    quests = window.SolarQuests ? window.SolarQuests.create(THREE, shadow.scene, els.shadowCanvas.parentElement, questHooks()) : null;
    ...
    document.getElementById('centerOffset').addEventListener('input', () => {
      if (weeklyPreview) { stopDay(); centerDayPlayback = false; updateAll(); }
    });
  ...
  function getSceneDate() {
    if (weeklyPreview && centerDayPlayback && cameraMode === 'center' && !explorer?.active) return getSelectedJSDate();
  ...
  function publishToolbar() {
    ...
    const ui = {
      date: formatDateShortISO(getSceneSettings().localDate), clock: explorer?.active ? formatTime(getSceneDate()) + ' · preview' : cameraMode === 'center' ? formatTime(getSceneDate()) + (centerDayPlayback ? ' · day at center' : day.kind === 'normal' ? ' · sunrise preview' : ' · polar noon preview') : document.getElementById('dayClock').textContent,
  ...
  function toolbarAction(action, value) {
    ...
    const dayAction = ['playBtn', 'minutes', 'dayStart', 'dayEnd', 'noonBtn', 'dialMorning', 'dialAfternoon', 'morningLight'].includes(action);
    if (weeklyPreview && !presenting && dayAction) {
      if (explorer?.active) setExplore(false);
      if (cameraMode === 'center') centerDayPlayback = true;
    }
    if (weeklyPreview && action === 'buildMode' && cameraMode === 'center') leaveCenterView();
    ...
    if (action === 'morningLight' && !presenting && (cameraMode !== 'center' || weeklyPreview) && !explorer?.active) {
    ...
    if (cameraMode === 'center' && ['playBtn','post','markDial','undoDial','minutes','dayStart','dayEnd','noonBtn','dialMorning','dialAfternoon'].includes(action) && !(weeklyPreview && dayAction)) return;
  ...
  // Level challenges read the same scene Sun as the lesson; they never edit the design.
  function updateQuest() {
    if (!quests?.quest || !shadow.scene) return;
    const sun = getSunPosition(getSceneDate(), state.lat, state.lon);
    quests.update({ design: monumentDesign, settings: getSceneSettings(), sun: window.SolarGeometry.sunDirection(sun.altitudeDeg, sun.compassDeg), sundial: isSundial() });
  }
  function questObserve(localDate, rule = observationRule) {
    if (document.body.classList.contains('presenting')) return;
    if (explorer?.active) setExplore(false);
    if (cameraMode === 'center') leaveCenterView();
    stopDay(); cancelMarkerPlacement(true);
    state.localDateISO = normalizeDateInput(localDate, state.localDateISO);
    els.dateInput.value = state.localDateISO;
    observationRule = rule;
    if (rule !== 'clock') { const observed = window.SolarDay.observe(observationSettings(), rule); state.minutes = observed ? observed.minutes : currentDay().noon; }
    else state.minutes = clamp(state.minutes, currentDay().start, currentDay().end);
    els.timeSlider.value = state.minutes;
    setSunMode(true); frameKey = ''; updateAll();
  }
  function questHooks() {
    return {
      render: () => { sceneDirty = true; },
      post: payload => window.parent.postMessage({ channel: 'forge.design-simulation.v1', ...payload }, window.location.origin),
      stepDays: days => questObserve(DateTime.fromISO(state.localDateISO).plus({ days }).toISODate()),
      season: key => { if (Object.hasOwn(seasons, key)) questObserve(seasonISO(seasons[key], getSelectedYear())); },
      noon: () => questObserve(state.localDateISO, 'noon'),
      snapshot: () => ({ localDate: state.localDateISO, minutes: state.minutes, rule: observationRule }),
      restore: snapshot => {
        questObserve(snapshot.localDate, snapshot.rule);
        if (snapshot.rule === 'clock') { state.minutes = snapshot.minutes; els.timeSlider.value = state.minutes; updateAll(); }
      },
      ...
      observeClock: minutes => {
        if (document.body.classList.contains('presenting')) return;
        stopDay(); observationRule = 'clock';
        const d = currentDay(); state.minutes = clamp(minutes, d.start, d.end); els.timeSlider.value = state.minutes;
        setSunMode(true); updateAll();
      },
    };
  }
  ...
  function animationLoop() {
    if (hostActive && quests?.tick(performance.now())) sceneDirty = true;
  ...
  function bindMonumentBridge() {
    ...
    window.addEventListener('message', (event) => {
      ...
      if (data.type === 'quest') { quests?.setQuest(data.quest ?? null, data.completed === true); updateAll(); return; }
      ...
      if (data.type === 'weekly-preview') {
        weeklyPreview = data.active === true; centerDayPlayback = false;
        document.body.classList.toggle('weekly-preview', weeklyPreview);
        resizeCanvases(); resizeShadowRenderer(); updateAll(); return;
      }
      ...
      if (data.type === 'restore') {
        setSunMode(true);
        let settings = data.capture.settings;
        if (weeklyPreview && settings && !Number.isFinite(settings.minutes)) {
          const observation = window.SolarDay.observe(settings, settings.observationRule ?? 'noon');
          if (observation) settings = { ...settings, minutes: observation.minutes };
        }
        if (weeklyPreview) {
          if (explorer?.active) setExplore(false);
          if (cameraMode === 'center') leaveCenterView();
